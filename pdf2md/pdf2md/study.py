...
NOTE_SYSTEM = (
    "あなたは研究者の学習コーチです。与えられた章から、研究に役立つ"
    "『自分用の凝縮ノート』を日本語で作ります。冗長な引用はせず要点を圧縮し、"
    "全体像は Mermaid 図で表し、研究への示唆を明確にします。"
)

NOTE_TEMPLATE = """次の章の本文から、研究向けの凝縮スタディノートを**日本語**で作ってください。
専門用語は残しつつ簡潔に。図は Obsidian で描画できる **Mermaid** で書いてください。

出力フォーマット(この見出しをそのまま使う):
## 全体像(図)
```mermaid
flowchart TD
  %% 主要概念の関係を5〜10ノードで。ラベルは短く。構文エラーの無い有効なMermaidにする
```
## 重要概念(定義つき)
- 用語 — 1行の定義
...
## 研究への示唆・アイデア
- (この内容が研究のどこに使えるか / 生まれる問い / 次に読むべき方向。2〜4点、具体的に)
...
def generate_note(body: str, model: str = "claude-opus-4-8", max_tokens: int = 3000):
