...
The pattern reverses under drip irrigation. At TzM summer the cross-source correlation drops to r = +0.16, and within the d0–3 bucket it becomes negative (r = −0.19, n = 281): in the days immediately following an irrigation event the surface 5 cm sensor reads progressively *drier* (post-event surface evaporation) while the SMAP root-zone reads progressively *wetter*. By d8+ the cross-source correlation recovers to r = +0.61 as both compartments equilibrate. This sign reversal is *consistent with* a depth inversion in soil moisture between the two compartments, in which an irrigation-derived store at intermediate depth wets the root-zone while the surface dries — the soil-physics signature predicted by the drip wetted-bulb framework (Skaggs et al., 2004; Cote et al., 2003).

We emphasise, however, that this SMAP signal cannot be claimed as a *direct* observation of the wetted bulb itself. SMAP L4 root-zone is a CLSM-modelled state propagated from the 0–5 cm L-band brightness-temperature assimilation rather than an independent measurement, and its 9 km footprint averages the TzM orchard with surrounding rainfed parcels; the orchard fraction of the SMAP pixel is well below 10 %, and the wetted bulbs themselves occupy < 0.1 % of the pixel area. The negative correlation could in principle also arise from confounds at the pixel scale — for example, sub-pixel mixing of irrigated and dryland signals, or vegetation-optical-depth effects at the L-band retrieval. What the SMAP comparison robustly establishes is the contrast between the rainfed control (r = +0.80 at Oran spring, where the two compartments move in lock-step) and the drip-irrigated orchard (r = −0.19 at TzM d0–3, where they decouple in sign within the first three days). This contrast is, by itself, evidence that the surface and root-zone moisture states diverge under drip in a way that they do not under rainfall. Pinning down the precise depth (10–30 cm) of the responsible store requires either in-situ soil-moisture profile sensors at the orchard or sub-pixel SAR-derived soil-moisture retrievals (Sentinel-1 at 10 m), both beyond the scope of this paper. Because neither SMAP-resolvable compartment probes the active 10–30 cm depth proposed by Skaggs and Cote, both miss the actual transpiration source — which is itself consistent with why satellite ET retrievals driven by either surface (LST, SMAP_surface) or root-zone (SMAP_rootzone) moisture systematically underestimate ET under drip irrigation.
...
### 4.1 Mechanism
Our results are *consistent with* the drip wetted-bulb mechanism (Skaggs et al., 2004; Cote et al., 2003) as the soil-physics explanation for the observed dynamics. The argument is built in two steps, of which only the first is a direct observational claim.

**Step 1 (directly observed):** at TzM under drip irrigation, EC LE is decoupled from the 0–5 cm surface SWC during the first three days after an irrigation event (SDS_d0-3 = +0.13, vs Oran rainfed spring SDS = +0.43) and recovers to coupling by d4–7 (SDS ≈ 0). The decoupling is fully explained by days_since_irrig — neither season nor VPD adds explanatory power beyond it (AIC ΔAIC = −66 to −153 for the days_since_irrig term vs VPD-only baseline). The 3–4 d timescale matches both the LE-recovery τ recovered from EC alone (analysis A v27; τ_TzM = 3.36 d) and the satellite-EC bias-decay τ (sections 3.6 and 3.9; τ_METv3 = 4.0–6.0 d). Some irrigation-derived moisture source therefore sustains transpiration for 3–4 days while the surface (0–5 cm) remains dry, and is exhausted on the same timescale; this rules out deep groundwater or deep-root storage, which would produce longer (seasonal) τ and would not be event-locked.

**Step 2 (inferred mechanism, not directly observed at the relevant depth):** the most parsimonious soil-physics description of such an irrigation-derived, 3–4 d-lifetime, vertically displaced storage is the *wetted bulb* — a localised, near-saturated zone formed at 10–30 cm depth around each drip emitter and depleted by root uptake (Skaggs et al., 2004; Cote et al., 2003). Almond root density is highest in this band (Fereres & Goldhamer, 1990), making it the most plausible transpiration source. We do not directly observe the wet bulb in our dataset (the only in-situ SWC sensor is at 5 cm), but every quantitative observation from this study — the d0–3 decoupling, the 3–4 d τ, the days_since_irrig dependence, the absence of seasonal trend — is consistent with this mechanism and inconsistent with the alternatives we tested. The wet-bulb interpretation should therefore be read as *the soil-physics framework that best organises our observations*, not as a directly demonstrated structure.

The SMAP comparison (section 3.7) provides additional, *indirect* evidence consistent with depth inversion: at rainfed Oran spring the in-situ 5 cm and SMAP root-zone observations move together (r = +0.80), whereas at TzM summer d0–3 they decouple in sign (r = −0.19). The contrast between sites is the robust signal — under drip the two soil-moisture compartments diverge in a way that they do not under rainfall — but the absolute negative correlation should not be read as a direct observation of the wet bulb at SMAP's 9 km footprint, in which the orchard occupies < 10 % of the area and the wetted bulbs themselves occupy < 0.1 %. SMAP L4 root-zone is moreover a CLSM-modelled state rather than a direct measurement at depth. Tight verification of the proposed 10–30 cm depth requires either in-situ profile sensors at the orchard or sub-pixel SAR-derived retrievals at the parcel scale, both of which we identify as priorities for follow-up work.
...
**Depth of the inferred wet bulb is not directly observed.** The in-situ SWC sensor at the orchard probes only the 0–5 cm surface, so the 10–30 cm depth band where the wet bulb is hypothesised by Skaggs et al. (2004) and Cote et al. (2003) is not instrumented in this study. Our claim of a wet-bulb mechanism is therefore an *indirect* inference from the observed irrigation-cycle structure of LE, days_since_irrig dominance in the AIC comparison, and the absence of seasonal or deep-root signatures — not a direct measurement at the relevant depth. The SMAP L4 root-zone is suggestive but cannot deliver decisive evidence at this scale: the 9 km footprint averages the TzM orchard (<10 % areal fraction) with surrounding rainfed parcels, and the L4 root-zone is itself a CLSM-modelled propagation of the 0–5 cm L-band signal rather than an independent observation at depth. Direct verification at the parcel scale requires either in-situ profile sensors (e.g., a 5/15/30/50 cm SWC chain) or sub-pixel soil-moisture retrievals from Sentinel-1 SAR backscatter (~10 m), the latter of which has been demonstrated to track irrigation events at field scale (Bazzi et al., 2019; El Hajj et al., 2017) and would allow resolved tracking of the wet bulb's lateral and (with multi-polarisation inversion) shallow vertical structure. We identify these direct depth-resolved measurements as a priority for follow-up work; they would either confirm the 10–30 cm wet bulb mechanism or motivate revisiting the moisture-source attribution.
