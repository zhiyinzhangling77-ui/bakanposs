...
def main():
    ...
    print("  留保：")
    print("   ・**`CSR_TAIR` は「気温」ではない**——`CSR_COLUMN_UNITS.csv` の定義は")
    print("     **『Chamber air temperature during flux measurement』＝チャンバー内の空気温度**")
    print("     である（周囲の気温は別列 `CSR_TAIR_AMB`）。`columns.csv` でも提供者の")
    print("     `Chambertemp` が `CSR_TAIR` に写されている。")
    print("     ＝**閉鎖チャンバー内の空気は、日射でチャンバー自体が温まる影響を受ける**。")
    print("     **地温の代用として使うのは、単に『別の媒体』であるだけでなく、")
    print("     測器そのものの影響が入りうる**＝**代用した対には印を付けて別に数える。**")
