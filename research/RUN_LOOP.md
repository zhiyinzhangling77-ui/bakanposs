...
-b, --branch NAME    回してよいブランチ (default claude/new-branch-creation-heq0i1)
    ...
    --allow "<tools>"  承認なしで通すツールを絞る（--yolo の代わり）
```

## ★ 権限 — 無人で回すなら避けて通れない

**非対話セッション（`-p`）には承認する人がいない。** 既定の `--permission-mode acceptEdits` では
`python3 script.py` や **`git commit` すら承認待ちのまま拒否される**。

実際に起きたこと（周1）: Claude は旗103 の道具を書き、FLAGS_LOG と SESSION_STATE も更新したが、
**`git add` / `git commit` が拒否されて全部が未コミットで残り**、ドライバが止めた。
編集だけ進んでコミットできないのは、この症状の典型。

対処は 2 つ:

```bash
# (a) 全部通す。無人放置の既定はこれ
research/run_loop.sh -n 30 --yolo

# (b) 絞る。何を通すか自分で決めたいとき
research/run_loop.sh -n 30 --allow 'Bash(git *) Bash(python3 *) Bash(nohup *) Read Write Edit Grep Glob'
```

(b) は安全側だが、**複合コマンド（`cat x && echo y`、パイプ）が一覧に合致せず落ちることがある**。
研究の解析は任意のコマンドを打つので、実用上は (a) になりやすい。

ドライバは、周が未コミットで終わったとき **権限が原因の可能性を名指しで警告する**。
ログ中に承認待ちの痕跡があれば周ごとにも警告を出す。
...
| ターン数 | 67 |
| 費用 | **$4.46** |
| 成果 | 旗103 の道具 1 本 + 道具の欠陥 #35 の発見 + 3 文書の更新 |

**30 周だと単純外挿で 5〜6 時間・$130 前後。** ただし実データを走らせる周はもっと長く、
判断ゲートに当たれば途中で止まる。**まず 2〜3 周で自分の数字を取ること。**
