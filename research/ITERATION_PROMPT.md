...
## ★1.5 この環境の新事実（旗114/118・2026-09-03・見落とすな）

- **この LOCAL 環境では `WebSearch`/`WebFetch` が動く**（旗118）。**旗92 の「文献に届かない」はコンテナ固有。**
  **＝GATE-07（一次文献の確認）は人間ゲートではない。自分で調べてよい。**
  **到達しやすい**：IOP（ERL）・Copernicus（BG/HESS）・PMC・PDF 直リンク。**403 になりやすい**：Wiley・Elsevier・ResearchGate。
  **要約に頼らず、到達できない文献は「未確認」と書く**（誇張を避ける側に倒す）。
- **ユーザー指示（GATE-11・最重要）**：「いけるところまで行く。**わかったことが新しい発見かどうかを、
  先行研究をしっかり調べて言う**」。**新規性の判定は `NOVELTY_ASSESSMENT.md` に主張ごとに書く（🟢/🟡/🔴/⚪）。**
  **⚪（未確認）を、到達しやすいソースで一つずつ潰すのが、当面の最優先の手である。**
- **GATE-10＝解決**（旗66 の同一地点選択は原理的でロジカル・ユーザー承認）。**旗85 の率訂正は正直に併記して反映。**
- **GATE-05＝方針確定**（`GATE05_ACQUISITION.md`）：**26組の大半は既に現地にある。追加取得（CA-Obs・US-SCf・
  US-RTS・US-SJ1）は DUA が要る人間ゲート**だが、**本命は既取得サイトでの「同一地点★率の直接測定」（旗80/86 の
  検定はこれから）＝手元で自走可能で、旗85 の 25〜29% 推定を実証し GATE-10 を裏打ちする。**
- **旗114 で `discover_chinaflux_sites` を修正済み**（CN-HbC の器混入）。**ChinaFlux は 65 サイト。**
- **★2026-09-07・重要**：**この自走ループは `run_loop.sh` の `--add-dir /mnt/hdd` により、
  非対話プロセスでも `/mnt/hdd`（実データ）を読める**（旗119/120 の「サンドボックス外で不可」は解消）。
  **＝実データ解析（★率の直接測定・手B/C の実検定・衛星接続の下調べ など）を自分で走らせてよい。**
  `.venv/bin/python`・`git`・`WebSearch`/`WebFetch` は `settings.local.json` と `--allowedTools` で承認済み。
  **周の最初の `ls /mnt/hdd` が通れば LOCAL。D 分類はゲートではない。**
  **文献側の ⚪ は旗134 で尽きた**ので、**次に価値が高いのは実データ解析**（GATE-05 の★率直接測定＝旗80/86 の
  「検定はこれから」、`GATE05_ACQUISITION.md`）と、**衛星接続の下調べ**（`/mnt/hdd/Dataset` の Oran_TzM スタックで
  θ→蒸発と春秋の非対称が衛星から見えるか）。**ユーザー向けの文章は書かない**（自然な日本語は対話側で整える）。
