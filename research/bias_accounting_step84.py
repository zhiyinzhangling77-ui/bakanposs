...
def main():
    ...
    print("  初版の前提（『全サイト座標が無いので総当たりできない』）は**もう当たらない**——")
    print("  旗85 が **837 サイト**で総当たりし、旗86 が**★で選ばずに 17 本を取得**した。")
    print("  ＝**『★で選ばず』群は、間接推定ではなく実測**である。\n")
    print("  **注意：対の数え方**——`チャンバー側に土壌温度が無い` 組は**ここに現れない**。")
    print("  旗86 では**カリフォルニアの 6 組ほか計 7 組**がそれで落ちている。")
    print("  ＝**下の『対の数』は、座標で作れた組の数ではなく、判定を試みられた組の数**。\n")
