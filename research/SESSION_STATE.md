...
## ★次にやれる残りの穴（優先順）
- ② TKY同一サイトのチャンバー（JP-Tak完全同一地点）＝Kishimoto-Mo氏へ外部連絡（COSORE v0.7未同梱, DOI 10.1007/s11284-015-1254-6）。埋まれば弧が完全に閉じる。
- ④残り 呼吸相乗(旗14)・θ→GER(旗31)のGER依存＝まだチャンバー未裏取り。
- ⑤残り ブートCI多サイト比較(θ→炭素の"何サイト有意")の族補正。
- ⑦ 構造限界（夏のみ・短記録・θ深度不統一）。
- synthesis.html に旗42(水分依存Q10復権)を反映するのも可。
