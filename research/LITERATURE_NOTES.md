...
> **⚠ 旗119 で「再発見」という語を取り下げた（本書末尾の訂正節を見よ）。以下は旗49 時点の記録。**
...
## 【旗119・訂正】上の「位置づけへの帰結」2 の**「DAMM の再発見」という語を取り下げる**

**上の記録は消さない**（旗49 時点でそう判断した事実は残す）。**訂正の理由と、置き換える文だけを足す。**

**理由**：**旗118 の一次確認で、Davidson et al. 2012（DAMM）は Harvard Forest の単一サイトで
trenched plot と実験室酵素アッセイにより構築・検証された過程モデルであり、多サイトの経験的検証を
含まないことが分かった**（`NOVELTY_ASSESSMENT.md` の確認記録を見よ）。
**＝機構の提案（DAMM）と、36 サイトでの経験的検証＋温度エイリアシングの分離（旗42/44）は、
同じ寄与の重複ではなく別種の寄与である。** **「再発見」は下げすぎだった。**

**置き換える文**（`MEASURED_ONLY_SPINE.md`【旗119・手F】と同一）：

> 土壌水分は見かけの温度感度を変える。機構としては DAMM（Davidson et al. 2012, GCB 18:371–384）が、
> 水分による酸素と可溶性炭素の拡散律速として説明を与えている（Harvard Forest の単一サイトで
> 構築・検証された過程モデルである）。本研究は、分割を通さないチャンバー観測 36 データセットで
> 水分依存の見かけ Q10 を経験的に検定し（18/36 で生存）、温度曲率によるエイリアシングを分離した。
> 符号の規則は未解明である。

**変えないもの**：**上の 1（O-information の新規性撤回）は維持する**——ただし
**残差分は「派生量監査だけ」ではなく「半時間サイト単位のタワー＋派生量監査＋分割の扱い」である**（旗118）。
**上の 3（方法論の監査が独自）も維持する。**

**作法として残すこと**：**自分の主張を削るために文献を使うと、削りすぎても誰も気づかない。**
**削る根拠にした文献は、一次で確認するまで暫定にしておく。**
