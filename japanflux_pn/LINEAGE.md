...
| 我々が使う手法 | 原典（要一次確認） |
|---|---|
| **O-information（系全体の相乗/冗長, Ω）** ★novelty の核 ✅**一次確認済→ K-1**（実装と式が一致） | **Rosas, F. et al. 2019. *Phys. Rev. E* 100, 032305. DOI:10.1103/PhysRevE.100.032305** |
...
## K. 追加で受領した一次資料（5 本）と、そこから抜き出した厳密な定義・式
**入手済みの一次資料**（§J の穴が埋まった）。以下は**実 PDF から抜粋**（捏造なし）。

### K-1. Rosas et al. 2019, *Phys. Rev. E* 100, 032305 — O-information ★novelty の核・実装と一致
- **定義（Def. 1, Eq. 4, 原文抜粋）**:
  $$\Omega(X^n) := \mathrm{TC}(X^n) - \mathrm{DTC}(X^n) = (n-2)H(X^n) + \sum_{j=1}^{n}\big[H(X_j) - H(X^n_{-j})\big]$$
- **符号の意味（原文）**: 「Ω>0 は依存性が"共有ランダムネス（冗長）"として効率的に説明できることを、
  Ω<0 は"集団的制約（相乗）"として見るのが良いことを示す」。TC＝total correlation、DTC＝dual TC。
- **我々の実装との一致**: `oinfo_analysis.py` の `o_information_indices` はこの Eq (4) そのもの
  （(n−2)H(全体)+Σ[H(単独)−H(除外)]）、符号判定（Ω<0=相乗）も原典どおり。→ **「O-info の定義は
  Rosas 2019 Eq (4) に厳密準拠」と引用できる**。⚠ ページは PDF で最終確認（Def.1 付近）。

### K-2. Ruddell & Kumar 2009, *WRR* 45, Part 1: Identification ★本研究の直接の親
- **原文抜粋**: 「transfer entropy を、時系列データからエコ水文変数**対**の情報流の**方向性・相対強度・
  統計的有意性・時間スケール**を頑健に測るために用いる。変数をノード、情報流を**重み付き有向リンク**と
  したものが**プロセスネットワーク**」。
- **非対称性（原文）**: T(X→Y) ≠ T(Y→X)、両者は**異なるラグ**で起こりうる（大きさと特徴ラグを特徴づける）。
- **有意性**: **shuffled surrogates**（X,Y をシャッフルしたにせ系列）で統計的有意性を判定。
- → 我々の TE・特徴ラグ・サロゲート有意性・ネットワーク構築は**この論文の方法に準拠**。

### K-3. Medlyn et al. 2011, *GCB* 17 — 気孔コンダクタンス（GPP/VPD の原典, §1 背景）
- **要点（原文）**: 最適気孔理論から**経験モデルと同形の統一モデル**を導出。気孔コンダクタンス
  $g_s \approx g_0 + 1.6\left(1+\dfrac{g_1}{\sqrt{D}}\right)\dfrac{A}{C_a}$（$D$=VPD）で、**VPD が上がると $g_s$ が
  下がる（$\propto 1/\sqrt{D}$）**。
- **生態系依存**: パラメータ **$g_1$ は生育温度で変わる**（熱帯〜亜寒帯で異なる）＝**VPD 応答は生態系/気候
  依存**。→ 我々の「森林で放射脱結合・感度は生態系依存」（fig7）の背景に直結。⚠ 式の係数は PDF で確認。

### K-4. Jung et al. 2020, *Biogeosciences* 17, 1343–1365 — FLUXCOM ★モデル対決
- **原文抜粋**: 「**全球 NEE の経年変動(IAV)は、逆解析や DGVM に比べて FLUXCOM では過小評価される**」。
  平均季節性はよく再現。→ 我々の「相乗/冗長・結合強度の**気候（年々）依存**」が、データ駆動 upscaling の
  弱点（IAV 過小）を**補う**という位置づけの直接根拠。DOI:10.5194/bg-17-1343-2020。

### K-5. Hodson et al. 2022（*WRR* 投稿, arXiv:2212.06566）— 情報理論で目的関数を選ぶ
- **要点**: モデル評価の目的関数（性能指標）の選択を**情報損失の最小化（AIC）**として定式化。
  → §H「情報理論によるモデル診断」の系譜（Nearing 2020 と並ぶ）。我々の「モデルの相互作用構造を
  情報で診断」路線を、目的関数選択の側から補強。

**まとめ（この 5 本で埋まった穴）**: §J の最重要 **Rosas 2019（O-info）が一次確認でき、我々の実装と式が
一致**。R&K 2009・Medlyn 2011・FLUXCOM・Hodson も一次資料が揃い、**背景（呼吸・気孔）・手法（TE/O-info）・
モデル対決（FLUXCOM/目的関数）の引用を実式・実文で固められる**。
